...
//                            userCallBack.onSocketMsgCallback(values[0]);
//                        }
//                    }

                    // 服务器发来其他客户端共享的位置信息
                    if (flag == ClientUtil.DATA_FLAG_LOCATION) {
                        if (mapCallBack != null) {
                    ...
                    // 服务器发来司机端的信息
                    if (flag == ClientUtil.DATA_FLAG_DRIVER) {
                        if (takeTaxiCallBack != null) {
                            takeTaxiCallBack.onSocketMsgCallback(values[0]);
                        }
                    }
     ...
     * 处理用户名设置的回调
     */
    private SocketTakeTaxiCallBack takeTaxiCallBack = null;

    public SocketTakeTaxiCallBack getTakeTaxiCallBack() {
        return takeTaxiCallBack;
    }

    public void setTakeTaxiCallBack(SocketTakeTaxiCallBack takeTaxiCallBack) {
        this.takeTaxiCallBack = takeTaxiCallBack;
    }

    public interface SocketTakeTaxiCallBack {
        void onSocketMsgCallback(String msg);
    }
}
